export const ar = {
  Add: "إضافة",
  Remove: "إزالة",
  Posts: "المنشورات",
  Description: "الوصف",
  Area: "المنطقة",
  Address: "العنوان",
  New: "جديد",
  "Qura — Your city, one feed": "قرى — مدينتك، في تدفق واحد",
  "Discover restaurants, events, jobs, apartments, and more — all in one local feed. Launching in Aswan, expanding worldwide.":
    "اكتشف المطاعم، الفعاليات، الوظائف، الشقق، وأكثر — كل ذلك في تدفق مدينتك. ننطلق من أسوان، ونتوسع حول العالم.",
  "Sign in": "تسجيل الدخول",
  "Sign out": "تسجيل الخروج",
  "Select language": "اختر اللغة",
  Language: "اللغة",
  "Select theme": "اختر المظهر",
  Theme: "المظهر",
  Light: "فاتح",
  Dark: "داكن",
  System: "النظام",
  "Your city,": "مدينتك،",
  "Create your profile": "أنشئ ملفك الشخصي",
  "Welcome back": "أهلاً بعودتك",
  "Sign in to keep your city moving.": "سجّل الدخول لتبقى مدينتك على تواصل.",
  "Forgot your password?": "نسيت كلمة المرور؟",
  "New to Qura?": "جديد على قرى؟",
  "Or continue with": "أو تابع باستخدام",
  "Continue with Google": "المتابعة باستخدام Google",
  Email: "البريد الإلكتروني",
  Password: "كلمة المرور",
  "Show password": "إظهار كلمة المرور",
  "Hide password": "إخفاء كلمة المرور",
  "Back to sign in": "العودة لتسجيل الدخول",
  "Join your city's feed — it takes less than a minute.":
    "انضم إلى تدفق مدينتك — الأمر يستغرق أقل من دقيقة.",
  "Full name": "الاسم الكامل",
  "At least 8 characters.": "8 أحرف على الأقل.",
  "Create my profile": "إنشاء ملفي الشخصي",
  "Already have an account?": "لديك حساب بالفعل؟",
  "By creating an account, you agree to our": "بإنشاء حساب، فإنك توافق على",
  "Terms of Service": "شروط الخدمة",
  and: "و",
  "Privacy Policy": "سياسة الخصوصية",
  "Enter the email on your account and we'll send you a reset link.":
    "أدخل البريد الإلكتروني المرتبط بحسابك وسنرسل لك رابط إعادة التعيين.",
  "You're signed in — send a reset link for this account, or change the email below to reset a different one.":
    "لقد سجّلت الدخول بالفعل — أرسل رابط إعادة التعيين لهذا الحساب، أو غيّر البريد الإلكتروني أدناه لإعادة تعيين كلمة مرور حساب آخر.",
  "Send reset link": "إرسال رابط إعادة التعيين",
  "Check your email": "تحقق من بريدك الإلكتروني",
  "We've sent a password reset link to":
    "لقد أرسلنا رابط إعادة تعيين كلمة المرور إلى",
  "It may take a minute to arrive.": "قد يستغرق وصوله دقيقة.",
  "Didn't get it?": "لم تصلك الرسالة؟",
  "Resend link": "إعادة إرسال الرابط",
  "Resend available in {{seconds}}s":
    "إعادة الإرسال متاحة خلال {{seconds}} ثانية",
  "Set a new password": "تعيين كلمة مرور جديدة",
  "Make it strong — you'll use it to sign back in.":
    "اجعلها قوية — ستستخدمها لتسجيل الدخول مجدداً.",
  "New password": "كلمة المرور الجديدة",
  "Confirm password": "تأكيد كلمة المرور",
  "Reset password": "إعادة تعيين كلمة المرور",
  "Passwords don't match.": "كلمتا المرور غير متطابقتين.",
  "This link has expired": "انتهت صلاحية هذا الرابط",
  "Password reset links are only valid for a little while. Request a new one to continue.":
    "روابط إعادة تعيين كلمة المرور صالحة لفترة قصيرة فقط. اطلب رابطاً جديداً للمتابعة.",
  "Request a new link": "طلب رابط جديد",
  "Password updated": "تم تحديث كلمة المرور",
  "Your password has been changed. You can now sign in.":
    "تم تغيير كلمة المرور الخاصة بك. يمكنك الآن تسجيل الدخول.",
  "Continue to sign in": "المتابعة لتسجيل الدخول",
  "Verify your email": "تأكيد بريدك الإلكتروني",
  "We've sent a verification link to": "لقد أرسلنا رابط التحقق إلى",
  "Click it to activate your profile and start posting.":
    "اضغط عليه لتفعيل ملفك الشخصي والبدء في النشر.",
  "Didn't get the email?": "لم تصلك رسالة التحقق؟",
  "Resend verification link": "إعادة إرسال رابط التحقق",
  "Wrong email address?": "بريد إلكتروني خاطئ؟",
  "Go back": "رجوع",
  "Already verified?": "تم التحقق مسبقاً؟",
  "Verifying your email…": "جارٍ التحقق من بريدك الإلكتروني…",
  "Hang tight, this only takes a second.":
    "لحظات فقط، الأمر لن يستغرق وقتاً طويلاً.",
  "You're verified!": "تم التحقق من حسابك!",
  "Your account is active. Time to explore your city.":
    "حسابك الآن مُفعّل. حان وقت استكشاف مدينتك.",
  "Verification failed": "فشل التحقق",
  "This link is invalid or has expired. Request a new verification email to continue.":
    "هذا الرابط غير صالح أو منتهي الصلاحية. اطلب رسالة تحقق جديدة للمتابعة.",
  "Please enter your email.": "الرجاء إدخال بريدك الإلكتروني.",
  "Enter a valid email address.": "الرجاء إدخال بريد إلكتروني صالح.",
  "Please enter your password.": "الرجاء إدخال كلمة المرور.",
  "Please enter your full name.": "الرجاء إدخال اسمك الكامل.",
  "Password must be at least 8 characters.":
    "يجب أن تتكون كلمة المرور من 8 أحرف على الأقل.",
  "Please confirm your password.": "الرجاء تأكيد كلمة المرور.",
  "An account with this email already exists. Try signing in instead.":
    "يوجد حساب بهذا البريد الإلكتروني مسبقاً. جرّب تسجيل الدخول بدلاً من ذلك.",
  "Invalid email.": "البريد الإلكتروني غير صحيح.",
  "Invalid password.": "كلمة المرور غير صحيحة.",
  "This account uses {{provider}} sign-in. Continue with {{provider}} instead.":
    "هذا الحساب يستخدم تسجيل الدخول عبر {{provider}}. تابع باستخدام {{provider}} بدلاً من ذلك.",
  "a connected provider": "أحد مزودي الدخول المرتبطين",
  'This email is linked to your {{provider}} account. Sign in with {{provider}}, or use "Forgot password" to set one.':
    'هذا البريد الإلكتروني مرتبط بحساب {{provider}} الخاص بك. سجّل الدخول باستخدام {{provider}}، أو استخدم "نسيت كلمة المرور" لتعيين واحدة.',
  "Your account has been suspended. Contact support for help.":
    "تم تعليق حسابك. تواصل مع الدعم للمساعدة.",
  "Something went wrong. Please try again.":
    "حدث خطأ ما. الرجاء المحاولة مرة أخرى.",
  "Please wait a moment before requesting another link.":
    "الرجاء الانتظار قليلاً قبل طلب رابط آخر.",
  "Something went wrong signing in with Google. Please try again.":
    "حدث خطأ أثناء تسجيل الدخول عبر Google. الرجاء المحاولة مرة أخرى.",
  "Google sign-in isn't available right now.":
    "تسجيل الدخول عبر Google غير متاح حالياً.",
  "Go to your profile": "الذهاب إلى ملفك الشخصي",
  Account: "الحساب",
  Profile: "الملف الشخصي",
  "Categories & News": "الفئات والأخبار",
  Settings: "الإعدادات",
  "About the app": "عن التطبيق",
  Verified: "تم التحقق",
  "Verify your email — Qura": "تأكيد بريدك الإلكتروني — قرى",
  "Reset your password — Qura": "إعادة تعيين كلمة المرور — قرى",
  "Hi {{name}},": "مرحباً {{name}}،",
  "Verify your email to activate your Qura profile:":
    "أكّد بريدك الإلكتروني لتفعيل ملفك الشخصي على قرى:",
  "This link expires in 24 hours. If you didn't create a Qura account, you can ignore this email.":
    "تنتهي صلاحية هذا الرابط خلال 24 ساعة. إذا لم تُنشئ حساباً على قرى، يمكنك تجاهل هذه الرسالة.",
  "Reset your Qura password:": "أعد تعيين كلمة مرور حسابك على قرى:",
  "This link expires in 1 hour. If you didn't request this, you can ignore this email.":
    "تنتهي صلاحية هذا الرابط خلال ساعة واحدة. إذا لم تطلب ذلك، يمكنك تجاهل هذه الرسالة.",
  Categories: "الفئات",
  Explore: "استكشف",
  "Find trusted businesses and services around you.":
    "اعثر على أنشطة تجارية وخدمات موثوقة من حولك.",
  "Sign in to save favorites and manage your business":
    "سجّل الدخول لمتابعة الأنشطة التجارية وإدارة نشاطك التجاري",
  "Create an account": "إنشاء حساب",
  Active: "نشط",
  Following: "المتابَعون",
  Businesses: "الأنشطة التجارية",
  Call: "اتصال",
  About: "نبذة",
  Event: "فعالية",
  News: "الأخبار",
  Business: "الأعمال",
  Type: "النوع",
  Title: "العنوان",
  Expires: "تاريخ الانتهاء",
  Post: "نشر",
  Follow: "متابعة",
  Notifications: "الإشعارات",
  Home: "الرئيسية",
  English: "الإنجليزية",

  followers: "متابع",
  following: "متابَع",
  "Threads with these words or phrases will be filtered out of your feed and notifications.":
    "سيتم تصفية المنشورات التي تحتوي على هذه الكلمات أو العبارات من خلاصتك وإشعاراتك.",
  "Edit profile": "تعديل الملف الشخصي",
  "Share profile": "مشاركة الملف الشخصي",
  "Link copied": "تم نسخ الرابط",
  "Profile link copied to clipboard.": "تم نسخ رابط الملف الشخصي.",
  Threads: "المنشورات",
  Replies: "الردود",
  Reposts: "إعادة النشر",
  "You haven't posted anything yet.": "لم تنشر أي شيء بعد.",
  "You haven't replied to anything yet.": "لم ترد على أي شيء بعد.",
  "You haven't reposted anything yet.": "لم تُعد نشر أي شيء بعد.",
  Followers: "المتابعون",
  "No followers yet.": "لا يوجد متابعون بعد.",
  "You're not following anyone yet.": "أنت لا تتابع أحداً بعد.",
  "Set a break reminder": "تعيين تذكير استراحة",
  "We'll remind you to take a break after you've been scrolling for a while.":
    "سنذكّرك بأخذ استراحة بعد قضاء بعض الوقت في التصفح.",
  "Private profile": "حساب خاص",
  "Only people you approve can see your posts and followers.":
    "فقط الأشخاص الذين توافق عليهم يمكنهم رؤية منشوراتك ومتابعيك.",
  "Who can @mention you": "من يمكنه الإشارة إليك",
  Everyone: "الجميع",
  "People you follow": "الأشخاص الذين تتابعهم",
  Privacy: "الخصوصية",
  "Hidden words": "الكلمات المخفية",
  "Automatically filter out words and phrases you don't want to see.":
    "قم تلقائياً بتصفية الكلمات والعبارات التي لا تريد رؤيتها.",
  "Profile updated.": "تم تحديث الملف الشخصي.",
  Username: "اسم المستخدم",
  Bio: "نبذة تعريفية",
  Save: "حفظ",
  "Break reminder updated.": "تم تحديث تذكير الاستراحة.",
  Off: "معطّل",
  minutes: "دقيقة",
  "Add a word or phrase": "أضف كلمة أو عبارة",
  "No hidden words yet.": "لا توجد كلمات مخفية بعد.",
  "Invite friends": "دعوة الأصدقاء",
  "Pause notifications": "إيقاف الإشعارات مؤقتاً",
  "Report a problem": "الإبلاغ عن مشكلة",
  "Log out": "تسجيل الخروج",
  "You won't receive push notifications while paused. You can turn them back on anytime.":
    "لن تتلقى إشعارات فورية أثناء الإيقاف المؤقت. يمكنك إعادة تفعيلها في أي وقت.",
  "Share your invite link — anyone who signs up through it joins your city's feed.":
    "شارك رابط دعوتك — أي شخص يسجل من خلاله سينضم إلى تدفق مدينتك.",
  "Copy invite link": "نسخ رابط الدعوة",
  Copied: "تم النسخ",
  "Invite link copied to clipboard.": "تم نسخ رابط الدعوة.",
  "Notifications paused.": "تم إيقاف الإشعارات مؤقتاً.",
  "Notifications resumed.": "تم استئناف الإشعارات.",
  "Paused until": "متوقفة حتى",
  "Turn notifications back on": "إعادة تفعيل الإشعارات",
  "For 8 hours": "لمدة 8 ساعات",
  "For 24 hours": "لمدة 24 ساعة",
  "For 1 week": "لمدة أسبوع",
  "Until I turn it back on": "حتى أقوم بإعادة تفعيلها",
  "This is a placeholder Privacy Policy. Qura is still in development — the real policy will be published here before launch.":
    "هذه سياسة خصوصية مؤقتة. لا يزال قرى قيد التطوير — سيتم نشر السياسة الفعلية هنا قبل الإطلاق.",
  "Thanks — we've received your report.": "شكراً — لقد استلمنا بلاغك.",
  "Describe what went wrong…": "صف ما حدث من خطأ…",
  "We read every report, but won't be able to follow up individually.":
    "نقرأ كل بلاغ، لكن لن نتمكن من المتابعة الفردية.",
  Submit: "إرسال",

  "Username must be at least 3 characters.":
    "يجب أن يتكون اسم المستخدم من 3 أحرف على الأقل.",
  "Username must be at most 30 characters.":
    "يجب ألا يتجاوز اسم المستخدم 30 حرفاً.",
  "Username can only contain lowercase letters, numbers, underscores, and periods.":
    "يمكن أن يحتوي اسم المستخدم على أحرف إنجليزية صغيرة وأرقام وشرطات سفلية ونقاط فقط.",
  "Bio must be at most 150 characters.":
    "يجب ألا تتجاوز النبذة التعريفية 150 حرفاً.",
  "Enter a word or phrase.": "أدخل كلمة أو عبارة.",
  "Must be at most 50 characters.": "يجب ألا يتجاوز 50 حرفاً.",
  "Please describe the problem in a bit more detail.":
    "الرجاء وصف المشكلة بمزيد من التفصيل.",
  "Must be at most 2000 characters.": "يجب ألا يتجاوز 2000 حرف.",

  "You need to sign in to do that.": "يجب عليك تسجيل الدخول للقيام بذلك.",
  "That username is already taken.": "اسم المستخدم هذا مُستخدم بالفعل.",
  "You can't follow yourself.": "لا يمكنك متابعة نفسك.",

  "Delete thread": "حذف المنشور",
  Unsave: "إلغاء الحفظ",
  "Say something…": "اكتب شيئاً…",
  "Remove image": "إزالة الصورة",
  "Add image": "إضافة صورة",
  Reply: "رد",
  Thread: "المنشور",
  "No replies yet.": "لا توجد ردود بعد.",
  Feed: "الصفحة الرئيسية",
  Aswan: "أسوان",
  Luxor: "الأقصر",
  Cairo: "القاهرة",
  Alexandria: "الإسكندرية",
  Qena: "قنا",
  Hurghada: "الغردقة",
  "Sharm El Sheikh": "شرم الشيخ",
  Sohag: "سوهاج",
  "Marsa Alam": "مرسى علم",
  "Switch city": "تبديل المدينة",
  "Coming soon": "قريباً",
  "Good morning": "صباح الخير",
  "Good afternoon": "مساء الخير",
  "Good evening": "مساء الخير",
  "Good night": "مساء الخير",
  "Search what's around you…": "ابحث عمّا حولك…",
  "For you": "لك",
  Relevant: "الأكثر صلة",
  Latest: "الأحدث",
  "Sort feed": "ترتيب الصفحة الرئيسية",
  "No threads yet. Be the first to post.":
    "لا توجد منشورات بعد. كن أول من ينشر.",
  "No threads yet.": "لا توجد منشورات بعد.",
  Search: "بحث",
  "Search businesses": "ابحث عن الأنشطة التجارية",
  "No people found.": "لم يتم العثور على أشخاص.",
  "Enter a valid image URL.": "أدخل رابط صورة صالحاً.",
  "Must be at most 500 characters.": "يجب ألا يتجاوز 500 حرف.",
  "More options": "خيارات إضافية",
  "Thread options": "خيارات المنشور",
  Edit: "تعديل",
  Delete: "حذف",
  Cancel: "إلغاء",
  "You can only edit your own threads.": "يمكنك فقط تعديل منشوراتك الخاصة.",
  "Thread updated.": "تم تحديث المنشور.",
  Share: "مشاركة",
  "Link copied to clipboard.": "تم نسخ الرابط.",
  "New thread": "منشور جديد",
  "What's new?": "ما الجديد؟",
  "Discard thread?": "تجاهل المنشور؟",
  "Save draft": "حفظ كمسودة",
  Discard: "تجاهل",
  "Keep editing": "متابعة التحرير",
  "Draft saved.": "تم حفظ المسودة.",
  "Thread discarded.": "تم تجاهل المنشور.",
  "Sign in to continue": "سجّل الدخول للمتابعة",
  "You need an account to do that.": "تحتاج إلى حساب للقيام بذلك.",
  "Loading…": "جارٍ التحميل…",
  Top: "الأفضل",
  Recent: "الأحدث",
  "Sort replies": "ترتيب الردود",
  "Edit thread": "تعديل المنشور",
  "Discard changes?": "تجاهل التغييرات؟",
  "Delete thread?": "حذف المنشور؟",
  "This can't be undone.": "لا يمكن التراجع عن هذا الإجراء.",
  "Something went wrong": "حدث خطأ ما",
  "We couldn't reach our servers. This is usually temporary — try again in a moment.":
    "تعذّر الوصول إلى خوادمنا. عادةً ما يكون هذا مؤقتاً — حاول مرة أخرى بعد قليل.",
  Reference: "المرجع",
  "Try again": "حاول مرة أخرى",
  "Up to 4 images.": "بحد أقصى 4 صور.",
  "Remove this image": "إزالة هذه الصورة",
  "Add photos": "إضافة صور",
  "Move earlier": "تحريك للأمام",
  "Move later": "تحريك للخلف",
  "Couldn't upload that image.": "تعذّر رفع هذه الصورة.",
  "Images must be under 8MB.": "يجب ألا يتجاوز حجم الصورة 8 ميغابايت.",
  "Please enter a business name.": "يرجى إدخال اسم النشاط التجاري.",
  "Choose a category.": "اختر تصنيفًا.",
  "Choose a city.": "اختر مدينة.",
  "Add to Qura": "أضِف إلى Qura",
  "This creates a new Qura profile connected to this Google Place. It doesn't verify that you own or manage the real business.":
    "سيؤدي هذا إلى إنشاء ملف Qura جديد متصل بهذا المكان على Google. هذا لا يثبت أنك تملك أو تدير هذا النشاط التجاري فعليًا.",
  "Your Qura profile is connected to this Google Place.":
    "تم ربط ملفك التجاري على Qura بهذا المكان على Google.",
  Continue: "متابعة",
  City: "المدينة",
  "You can only edit your own business profiles.":
    "يمكنك فقط تعديل ملفات أعمالك التجارية.",
  "Business profiles": "الملفات التجارية",
  "Business name": "اسم النشاط التجاري",
  "Create business profile": "إنشاء ملف تجاري",
  "No business profiles yet.": "لا توجد ملفات تجارية بعد.",
  "Add a business profile": "إضافة ملف تجاري",
  "Manage the pages you post as — a business profile works exactly like your own, with its own posts, followers, and profile page.":
    "أدر الصفحات التي تنشر باسمها — يعمل الملف التجاري تماماً مثل ملفك الشخصي، وله منشوراته ومتابعوه وصفحته الخاصة.",
  "Business profile created.": "تم إنشاء الملف التجاري.",
  "Business profile updated.": "تم تحديث الملف التجاري.",
  "Business profile deleted.": "تم حذف الملف التجاري.",
  "Delete business profile": "حذف الملف التجاري",
  "Delete this business profile?": "حذف هذا الملف التجاري؟",
  "This will permanently delete this business profile and everything it posted.":
    "سيؤدي هذا إلى حذف هذا الملف التجاري وكل ما نشره نهائياً.",
  "You can't follow your own business profile.":
    "لا يمكنك متابعة ملفك التجاري الخاص.",
  "Post as": "النشر باسم",
  You: "أنت",
  "Business profiles can only post — not reply, follow, or like.":
    "يمكن للملفات التجارية النشر فقط — لا يمكنها الرد أو المتابعة أو الإعجاب.",
  "Switch profile": "تبديل الملف الشخصي",
  "Business profile": "الملف التجاري",
  "No businesses found.": "لم يتم العثور على أنشطة تجارية.",
  "Enter a valid Google place id.": "أدخل معرّف مكان صالح من Google.",
  "This Google place is already connected to another Qura business profile.":
    "هذا المكان على Google متصل بالفعل بملف تجاري آخر على Qura.",
  "This business is already connected to a different Google place. Disconnect it first.":
    "هذا النشاط التجاري متصل بالفعل بمكان مختلف على Google. قم بفصله أولاً.",
  "Set up your business profile before connecting a Google place.":
    "أكمل إعداد ملفك التجاري قبل ربط مكان من Google.",
  "Places powered by Google": "الأماكن مقدَّمة من Google",
  "Enter a valid Google place.": "يرجى اختيار مكان صالح من Google.",
  "Couldn't search Google right now. Try again shortly.":
    "تعذّر البحث في Google حالياً. حاول مرة أخرى بعد قليل.",
  "Connected to Google": "متصل بـ Google",
  "View Google Place": "عرض المكان على Google",
  Disconnect: "قطع الاتصال",
  "Disconnect Google Place?": "قطع الاتصال بمكان Google؟",
  "Your Qura profile, reviews, posts, followers, and other data will not be deleted — only this branch's connection to this Google Place is removed.":
    "لن يتم حذف ملفك التجاري على Qura أو تقييماتك أو منشوراتك أو متابعيك أو أي بيانات أخرى — سيُزال فقط اتصال هذا الفرع بهذا المكان على Google.",
  "Add a Google Place": "إضافة مكان Google",
  "Start from a Google Place": "ابدأ من مكان على Google",
  "Find your business on Google": "ابحث عن نشاطك التجاري على Google",
  "Pick your listing to fill in your name, address, and a suggested category — you can still edit everything before saving.":
    "اختر ملفك لملء الاسم والعنوان وتصنيف مقترح — يمكنك تعديل كل شيء قبل الحفظ.",
  "You already have a business connected to this Google Place.":
    "لديك بالفعل نشاط تجاري متصل بهذا المكان على Google.",
  "Google Places": "أماكن Google",
  "Connect Google Place": "ربط مكان من Google",
  "Connect your Google Place": "اربط مكانك على Google",
  "Search for your business on Google to link it to this Qura profile.":
    "ابحث عن نشاطك التجاري على Google لربطه بهذا الملف على Qura.",
  "Search for your business...": "ابحث عن نشاطك التجاري...",
  "No places found.": "لم يتم العثور على أماكن.",
  "Is this your business? This links your Qura profile to this Google Place — it doesn't verify or transfer ownership of the Google listing.":
    "هل هذا نشاطك التجاري؟ سيؤدي هذا إلى ربط ملفك على Qura بهذا المكان على Google — لا يُثبت هذا ولا ينقل ملكية القائمة على Google.",
  Connect: "ربط",
  Back: "رجوع",
  "Google Place connected": "تم ربط مكان Google",
  "This Google Place is also connected to another Qura business profile. Our team has been notified for review.":
    "هذا المكان على Google متصل أيضاً بملف تجاري آخر على Qura. تم إبلاغ فريقنا للمراجعة.",
  "This connection lets Qura show Google information alongside your profile.":
    "يتيح هذا الاتصال لـ Qura عرض معلومات Google بجانب ملفك التجاري.",
  Done: "تم",
  "Google Place": "مكان Google",
  "Google reviews": "تقييم على Google",
  Rating: "التقييم",
  "Show hours": "عرض المواعيد",
  "Temporarily closed": "مغلق مؤقتاً",
  "Permanently closed": "مغلق نهائياً",
  "Google information isn't available right now.":
    "معلومات Google غير متاحة حالياً.",
  "This Google information may be out of date.":
    "قد تكون معلومات Google هذه قديمة.",
  "Related via Google": "ذو صلة عبر Google",
  "Google Place Conflicts": "تعارضات أماكن Google",
  "Every event where a Qura business connected to a Google Place that another business already connects to. Connecting is always allowed — this list is for review, not enforcement.":
    "كل حدث اتصل فيه نشاط تجاري على Qura بمكان على Google متصل به نشاط آخر بالفعل. الاتصال مسموح به دائماً — هذه القائمة للمراجعة فقط وليست للمنع.",
  "No conflicts recorded.": "لا توجد تعارضات مسجّلة.",
  "Unknown business": "نشاط تجاري غير معروف",
  "Load more": "تحميل المزيد",
  Favorites: "المفضلة",
  "Threads you like will show up here.": "ستظهر هنا المنشورات التي أعجبتك.",
  "Please enter a cuisine type.": "يرجى إدخال نوع المطبخ.",
  "Please enter a specialty.": "يرجى إدخال التخصص.",
  "Must be at most 60 characters.": "يجب ألا يتجاوز 60 حرفاً.",
  "Must be at most 200 characters.": "يجب ألا يتجاوز 200 حرف.",
  "Must be at most 30 characters.": "يجب ألا يتجاوز 30 حرفاً.",
  "Enter a valid URL.": "أدخل رابطاً صالحاً.",
  "Business type": "نوع النشاط التجاري",
  Category: "الفئة",
  Info: "معلومات",
  "Recent searches": "عمليات البحث الأخيرة",
  Clear: "مسح",
  "No businesses in this category yet.":
    "لا توجد أنشطة تجارية في هذه الفئة بعد.",
  None: "لا شيء",
  Restaurant: "مطعم",
  Doctor: "طبيب",
  Cuisine: "نوع المطبخ",
  "Price range": "نطاق السعر",
  "Not set": "غير محدد",
  "Opening hours": "ساعات العمل",
  "Menu URL": "رابط القائمة",
  "Delivery available": "التوصيل متاح",
  "Reservations available": "الحجز متاح",
  Specialty: "التخصص",
  "Clinic address": "عنوان العيادة",
  "Working hours": "ساعات العمل",
  "Appointment phone": "هاتف الحجز",
  "Consultation fee": "رسوم الاستشارة",
  "Accepts insurance": "يقبل التأمين",
  "View menu": "عرض القائمة",
  Details: "التفاصيل",
  "Must be at most 300 characters.": "يجب ألا يتجاوز 300 حرف.",
  "Food & Drinks": "الطعام والمشروبات",
  Health: "الصحة",
  Beauty: "الجمال",
  Creative: "الإبداع",
  Shopping: "التسوق",
  "Home Services": "خدمات المنزل",
  Automotive: "السيارات",
  Education: "التعليم",
  Tourism: "السياحة",
  "Professional Services": "خدمات مهنية",
  "Real Estate": "العقارات",
  Jobs: "الوظائف",
  Marketplace: "السوق",
  Community: "المجتمع",
  Events: "الفعاليات",
  Government: "الحكومة",
  Emergency: "الطوارئ",
  Lifestyle: "نمط الحياة",
  Mon: "إثنين",
  Tue: "ثلاثاء",
  Wed: "أربعاء",
  Thu: "خميس",
  Fri: "جمعة",
  Sat: "سبت",
  Sun: "أحد",
  Closed: "مغلق",
  Today: "اليوم",
  "All day": "طوال اليوم",
  "Custom hours": "ساعات مخصصة",
  "Add time range": "إضافة فترة زمنية",
  "Copy to all days": "نسخ لكل الأيام",
  Open: "مفتوح",
  Available: "متاح",
  Accepted: "مقبول",
  Delivery: "التوصيل",
  Reservations: "الحجوزات",
  Insurance: "التأمين",
  Reviews: "التقييمات",
  "Write a review": "اكتب تقييماً",
  "Edit your review": "عدّل تقييمك",
  "Update review": "تحديث التقييم",
  "Submit review": "إرسال التقييم",
  "Share details of your experience (optional)": "شارك تفاصيل تجربتك (اختياري)",
  "Please select a rating.": "يرجى اختيار تقييم.",
  "You can't review your own business.": "لا يمكنك تقييم نشاطك التجاري الخاص.",
  "No reviews yet.": "لا توجد تقييمات بعد.",
  "Review updated.": "تم تحديث التقييم.",
  "Review submitted.": "تم إرسال التقييم.",
  "Your review": "تقييمك",
  "Delete this review?": "حذف هذا التقييم؟",
  "Review deleted.": "تم حذف التقييم.",
  "Review options": "خيارات التقييم",
  "Location type": "نوع الموقع",
  "Area or address": "المنطقة أو العنوان",
  "Map coordinates": "إحداثيات الخريطة",
  Latitude: "خط العرض",
  Longitude: "خط الطول",
  "Location details": "تفاصيل الموقع",
  "e.g. 2nd floor, blue door": "مثال: الطابق الثاني، الباب الأزرق",
  "Phone numbers": "أرقام الهاتف",
  "Add phone number": "إضافة رقم هاتف",
  "Location & contact": "الموقع والتواصل",
  Location: "الموقع",
  Phone: "الهاتف",
  "Open in Google Maps": "فتح في خرائط جوجل",
  "Please enter an area or address.": "يرجى إدخال منطقة أو عنوان.",
  "Enter a valid phone number.": "أدخل رقم هاتف صالحاً.",
  "Up to 5 phone numbers.": "بحد أقصى 5 أرقام هواتف.",
  "Social links": "روابط التواصل الاجتماعي",
  "Add your Instagram, Facebook, X, TikTok, or website link.":
    "أضف رابط إنستغرام أو فيسبوك أو إكس أو تيك توك أو موقعك الإلكتروني.",
  "Add social link": "إضافة رابط تواصل اجتماعي",
  "Up to 6 social links.": "بحد أقصى 6 روابط تواصل اجتماعي.",
  Instagram: "إنستغرام",
  X: "إكس",
  Facebook: "فيسبوك",
  TikTok: "تيك توك",
  YouTube: "يوتيوب",
  LinkedIn: "لينكد إن",
  Snapchat: "سناب شات",
  Pinterest: "بينترست",
  Telegram: "تيليغرام",
  WhatsApp: "واتساب",
  Website: "الموقع الإلكتروني",
  "Please enter a location description.": "يرجى إدخال وصف للموقع.",
  "Enter both latitude and longitude, or neither.":
    "أدخل خط العرض وخط الطول معاً، أو اتركهما فارغين.",
  "Exact coordinates (optional)": "إحداثيات دقيقة (اختياري)",
  "e.g. Downtown Aswan, near the Nubian Museum — 2nd floor, blue door":
    "مثال: وسط أسوان، بالقرب من متحف النوبة — الطابق الثاني، الباب الأزرق",
  "Your session expired. Please sign in again.":
    "انتهت صلاحية جلستك. يرجى تسجيل الدخول مرة أخرى.",
  "Change pin": "تغيير الموقع",
  "Pick location on map": "اختر الموقع على الخريطة",
  "This is helpful": "هذا مفيد",
  "This is not helpful": "هذا غير مفيد",
  "Not helpful": "غير مفيد",
  General: "عام",
  "Something to do together": "حاجة نعملها مع بعض",
  "My experience": "تجربتي",
  Question: "سؤال",
  Offer: "عرض",
  Announcement: "إعلان",
  Alert: "تنبيه",
  "All categories": "كل التصنيفات",
  "Image uploads aren't set up yet.": "رفع الصور غير مفعّل بعد.",
  "Only JPEG, PNG, WebP, or GIF images are allowed.":
    "يُسمح فقط بصور JPEG أو PNG أو WebP أو GIF.",
};
