...
export default async function ResetPassword({ params }: ResetPasswordProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center overflow-auto">
      <Link href={Paths.Home} className={cn(buttonVariants({ variant: "ghost" }), "absolute left-4 top-4 gap-2 rtl:flex-row-reverse")}>
        <Icons.chevronLeft />
        {c["back home"]}
      </Link>
      ...
      <section className="container flex w-full max-w-sm flex-col justify-center gap-5">
        <div className="flex flex-col gap-2 text-center">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">{c["reset password"]}</h1>
          <p className="text-sm text-muted-foreground">{c["enter a new strong password twice."]}</p>
        </div>

        <div>
          <Form infiniteLoading validation="reset-password" onSubmit={resetPassword} useForm={{ defaultValues: { token } }}>
            <div className="space-y-2">
              <FormInputField type="password" label={db["users"]["password"]["password"]} field={{ name: "password" }} />
              <FormInputField type="password" label={db["users"]["password"]["confirm password"]} field={{ name: "confirmPassword" }} />
            ...
            <p className="text-center text-sm text-muted-foreground">
              <Link href={Paths.Login} className="underline underline-offset-4 hover:text-primary">
