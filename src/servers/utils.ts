...
export function createServerAction<T, R>(actionFn: (data: T) => Promise<ServerActionResult<R>>, options?: { defaultMessage?: keyof Dictionary["actions"] }) {
  return async (data: T): Promise<ServerActionResult<R>> => {
    const { actions: c } = await getDictionary();
    const defaultMessage = options?.defaultMessage ? c[options?.defaultMessage] : null;
    ...
    try {
      ...
      if (error instanceof z.ZodError) return { ok: false, zodIssues: error?.issues };

      return {
        ok: false,
        message: error["message"] ?? defaultMessage ?? c["An unexpected error occured, please try again later."],
      };
    }
  };
}
